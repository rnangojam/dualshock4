...
from sensor_msgs.msg import BatteryState, Joy, JoyFeedbackArray, Imu, JoyFeedback
from ds4_driver_msgs.msg import Feedback, Report, Status
...
class ControllerRos(Controller):
    def __init__(self, node: Node):
        ...
        if self.use_standard_msgs:
            self.pub_report = self.node.create_publisher(Report, "raw_report", qos_profile)
            self.pub_battery = self.node.create_publisher(BatteryState, "battery", qos_profile)
            self.pub_joy = self.node.create_publisher(Joy, "joy", qos_profile)
            self.pub_imu = self.node.create_publisher(Imu, "imu", qos_profile)
            self.sub_feedback = self.node.create_subscription(
                JoyFeedbackArray, "set_feedback", self.cb_joy_feedback, qos_profile
            )

            if self._autorepeat_rate != 0:
                period = 1.0 / self._autorepeat_rate
                self.timer = self.node.create_timer(period, self.cb_joy_pub_timer)
        else:
            self.pub_status = self.node.create_publisher(Status, "status", qos_profile)
            self.sub_feedback = self.node.create_subscription(
                Feedback, "set_feedback", self.cb_feedback, qos_profile
            )
